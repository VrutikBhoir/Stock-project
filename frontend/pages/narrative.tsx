...
export default function NarrativePage() {
  ...
  const [investmentGoal, setInvestmentGoal] = useState<
    "Growth" | "Income" | "Capital Preservation" | "Speculative"
  >("Growth");
  const [inferredHorizon, setInferredHorizon] = useState<string>("");
  const [horizonJustification, setHorizonJustification] = useState<string>("");
  ...
  // 🧠 AI-Powered Time Horizon Inference
  const inferTimeHorizon = (
    investorType: string,
    investmentGoal: string
  ): { horizon: string; justification: string } => {
    // Rule 1: Capital Preservation always enforces Long-term
    if (investmentGoal === "Capital Preservation") {
      return {
        horizon: "Long-term",
        justification: "Capital preservation requires a long-term approach (3-10 years) to weather market volatility and protect principal."
      };
    }

    // Rule 2: Speculative/Trading goals override to Short-term
    if (investmentGoal === "Speculative") {
      return {
        horizon: "Short-term",
        justification: "Speculative trading goals focus on short-term price movements (days to weeks) to capture quick opportunities."
      };
    }

    // Rule 3: Income goals bias toward Long-term
    if (investmentGoal === "Income") {
      if (investorType === "Aggressive") {
        return {
          horizon: "Medium-term",
          justification: "Income strategy with aggressive profile suits medium-term (1-3 years) for dividend growth while maintaining flexibility."
        };
      }
      return {
        horizon: "Long-term",
        justification: "Income-focused strategies work best with long-term holdings (3-10 years) to maximize dividend compounding and stability."
      };
    }

    // Rule 4: Growth goals - depends on investor type
    if (investmentGoal === "Growth") {
      if (investorType === "Conservative") {
        return {
          horizon: "Long-term",
          justification: "Conservative growth investors benefit from long-term positions (3-10 years) to achieve steady appreciation with lower risk."
        };
      }
      if (investorType === "Aggressive") {
        return {
          horizon: "Short-term",
          justification: "Aggressive growth strategy targets short-term opportunities (weeks to months) to maximize rapid capital appreciation."
        };
      }
      return {
        horizon: "Medium-term",
        justification: "Balanced growth approach works best with medium-term horizon (1-3 years) to capture meaningful upside while managing risk."
      };
    }

    // Default based on investor type
    switch (investorType) {
      case "Conservative":
        return {
          horizon: "Long-term",
          justification: "Conservative investors focus on long-term wealth building (3-10 years) to minimize volatility and preserve capital."
        };
      case "Aggressive":
        return {
          horizon: "Short-term",
          justification: "Aggressive investors seek short-term gains (weeks to months) with higher risk tolerance for significant price swings."
        };
      default:
        return {
          horizon: "Medium-term",
          justification: "Balanced approach suits medium-term horizon (1-3 years) for moderate growth with acceptable risk levels."
        };
    }
  };

  // Update inferred horizon whenever investor type or goal changes
  useEffect(() => {
    const { horizon, justification } = inferTimeHorizon(investorType, investmentGoal);
    setInferredHorizon(horizon);
    setHorizonJustification(justification);
  }, [investorType, investmentGoal]);
  ...
  const runNarrative = async () => {
    ...
    try {
      // 🧠 Infer time horizon automatically based on profile
      const { horizon, justification } = inferTimeHorizon(investorType, investmentGoal);
      setInferredHorizon(horizon);
      setHorizonJustification(justification);
  ...
  // ✅ NEW: Map time horizon keys to human-readable duration labels
  ...
  const investmentGoalDescriptions = {
    ...
    "Capital Preservation": "Risk-aware analysis. Minimizing volatility focus.",
    Speculative: "Short-term tactical analysis. Quick momentum evaluation.",
  };
        ...
        <section className="card input-card">
          ...
          <div className="form-grid">
            ...
            <div className="form-group full-width">
              <label>Select Stock <span className="required">*</span></label>
              <SectorStockSelector />
            </div>
            ...
            {/* Investment Goal */}
            <div className="form-group">
              <label>Primary Investment Goal <span className="required">*</span></label>
              <select
              ...
              >
                ...
                <option value="Capital Preservation">Capital Preservation (Risk Minimization)</option>
                <option value="Speculative">Speculative (Short-term Tactics)</option>
              </select>
            ...
            {/* AI-Inferred Time Horizon Display */}
            {inferredHorizon && (
              <div className="form-group full-width" style={{ 
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
                  ...
                  <label style={{ margin: 0, color: '#a78bfa', fontWeight: 600 }}>AI-Inferred Time Horizon</label>
                </div>
                <div style={{ 
                  fontSize: '18px', 
                  fontWeight: 600, 
          ...
          <>
            ...
            {result.narrative.sections && (
              <>
                ...
                {/* ✅ UPDATED: For You card — now includes Expected Duration badge */}
                <section className="card narrative-card for-you">
                  ...
                  <div className="investor-recommendation">
                    <div className="recommendation-badge">
                      {result?.investor_context?.recommendation || 'N/A'}
                    </div>

                    {/* ✅ NEW: Expected Duration Badge */}
                    <div className="duration-badge">
                      <span className="duration-icon">⏱️</span>
                      <div className="duration-text">
                        <span className="duration-label">Recommended Duration</span>
                        <span className="duration-value">{inferredHorizon}</span>
                      </div>
                    </div>
