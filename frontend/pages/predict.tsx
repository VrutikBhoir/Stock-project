...
const IntradayChart = dynamic(
  () => import("../components/dashboard/IntradayChart"),
  { ssr: false }
);
...
function PredictPage() {
  ...
  const isIndex = stock?.instrumentType === "INDEX";
  ...
  // Prediction only starts when the user clicks ANALYZE — no auto-fetch on stock change
  ...
  const fetchPrediction = useCallback(async (overrideSymbol?: string) => {
    ...
    if (!targetSymbol || targetSymbol.trim() === "") {
      setError("Select/Enter a stock ticker symbol ");
      return;
    }
    if (stock?.instrumentType === "INDEX") {
      setError(null);              // clear old stock errors
      setActiveTab("charts");      // force charts tab
      return;                      // stop stock prediction API call
    }
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/predict/${targetSymbol}?steps=${steps}&investment_horizon=${investmentHorizon}`,
        {
          method: "GET",
          headers: {
  ...
  const smaData = indicators?.sma_20.map((val, i) => ({
    x: new Date(indicators.dates[i]).getTime(),
    y: val
  })).slice(-100) || [];

  const emaData = indicators?.ema_20.map((val, i) => ({
    x: new Date(indicators.dates[i]).getTime(),
    y: val
  })).slice(-100) || [];
  ...
  const smaEmaOptions: ApexOptions = {
    ...
    yaxis: { labels: { formatter: (val: number) => `$${val.toFixed(2)}`, style: { colors: "#64748b" } } },
    xaxis: { type: "datetime", labels: { show: false }, axisTicks: { show: false }, tooltip: { enabled: false } },
    grid: { borderColor: "#1e293b", strokeDashArray: 4 },
    legend: { show: true, position: 'top', horizontalAlign: 'left' },
    tooltip: { theme: "dark", shared: true, x: { format: "dd MMM yyyy" }, y: { formatter: (val: number) => `$${val.toFixed(2)}` } }
  };

  const smaEmaSeries = [
    { name: "Close Price", type: "line", data: historicalData.slice(-100) },
    { name: "SMA (20)", type: "line", data: smaData },
    { name: "EMA (20)", type: "line", data: emaData }
  ];

  const bollingerOptions: ApexOptions = {
    chart: { type: "line", height: 200, background: "transparent", toolbar: { show: false } },
    theme: { mode: "dark" },
    stroke: { width: [2, 1, 1], dashArray: [0, 3, 3], curve: "smooth" },
  ...
  const bollingerSeries = [
    { name: "Close Price", type: "line", data: historicalData.slice(-100) },
    { name: "Upper Band", type: "line", data: bbUpperData },
    { name: "Lower Band", type: "line", data: bbLowerData }
  ];

  const priceChange = data && data.reference_price
  ? data.live_price - data.reference_price
  : 0;

const priceChangePercent = data && data.reference_price
  ? (priceChange / data.reference_price) * 100
  : 0;
  ...
  <div className="days-input">
    ...
    <input
      ...
      value={steps}
      onChange={(e) =>
        setSteps(Math.min(30, Math.max(1, Number(e.target.value))))
      }
      min="1"
      max="30"
    />
...
{investmentHorizon === "medium_term" && (
  <div className="days-input">
    ...
    <input
      ...
      value={steps}
      onChange={(e) =>
        setSteps(Math.min(12, Math.max(1, Number(e.target.value))))
      }
      min="1"
      max="12"
    />
...
{investmentHorizon === "long_term" && (
  <div className="days-input">
    ...
    <input
      ...
      value={steps}
      onChange={(e) =>
        setSteps(Math.min(5, Math.max(1, Number(e.target.value))))
      }
      min="1"
      max="5"
    />
  ...
  investmentHorizon === "short_term"
    ...
    : investmentHorizon === "medium_term"
    ? "Predicted Price (Medium-Term)"
    : "Predicted Price (Long-Term)"
}
                value={`$${(
  investmentHorizon === "short_term"
    ? data.predicted_t1
    ...
    : data.forecast[data.forecast.length - 1]?.price
                ...
                <>
                  <div className="chart-card">
                    <div className="chart-header small">
                      <h3>Moving Averages - SMA & EMA (20)</h3>
                    </div>
                    <Chart options={smaEmaOptions} series={smaEmaSeries} height={200} type="line" />
                  </div>

                  <div className="chart-card">
                    <div className="chart-header small">
                      <h3>Bollinger Bands (20, 2std)</h3>
                    </div>
                    <Chart options={bollingerOptions} series={bollingerSeries} height={200} type="line" />
                  </div>
          ...
          {activeTab === 'analysis' && analysis && !isIndex &&(
                ...
                <div className="entry-exit-grid">
                  ...
                  <div className="exit-section">
                    ...
                    <div className="price-point target">
                      ...
                      {analysis.entry_exit_strategy.exit_points.target !== null
  ? `$${analysis.entry_exit_strategy.exit_points.target.toFixed(2)}`
  : "—"}
                    </div>
                    <div className="price-point stop-loss">
                      <span className="label">Stop Loss</span>
                      <span className="value">
  {analysis.entry_exit_strategy.exit_points.stop_loss !== null
    ? `$${analysis.entry_exit_strategy.exit_points.stop_loss.toFixed(2)}`
    : "—"}
</span>
                    </div>
                  </div>
