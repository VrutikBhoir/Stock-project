...
export default function EventImpact() {
  ...
  const analyze = async () => {
    ...
    if (!stockSymbol.trim() || !event.trim()) {
  ...
  const formatStrength = (value: string) => value.trim().toLowerCase();
  const formatRisk = (value: string) => value.trim().toLowerCase();

  const getReasonLine = (strength: string, risk: string) => {
    const strengthValue = formatStrength(strength);
    const riskValue = formatRisk(risk);

    const strengthPhrase =
      strengthValue === "high" ? "High impact strength" :
        strengthValue === "medium" ? "Moderate impact strength" :
          "Low impact strength";

    const riskPhrase =
      riskValue === "low risk" ? "a controlled risk environment" :
        riskValue === "moderate risk" ? "a moderate risk environment" :
          "a high risk environment";

    return `${strengthPhrase} and ${riskPhrase}.`;
  };

  const getReevaluationLine = (strength: string, risk: string) => {
    const strengthValue = formatStrength(strength);
    const riskValue = formatRisk(risk);

    if (riskValue === "low risk" && strengthValue !== "high") {
      return "Re-evaluate if risk or impact strength rises.";
    }

    if (riskValue === "low risk" && strengthValue === "high") {
      return "Re-evaluate if risk rises or impact strength fades.";
    }

    if (riskValue === "moderate risk") {
      return "Re-evaluate if risk escalates or strength improves.";
    }

    return "Re-evaluate if risk eases or impact strength shifts.";
  };

  const getInvestorSuggestion = (impact: string, strength: string, risk: string) => {
    const impactValue = impact.trim().toLowerCase();
    const strengthValue = formatStrength(strength);
    const riskValue = formatRisk(risk);

    if (
      impactValue === "positive impact" &&
      strengthValue === "high" &&
      riskValue === "low risk"
    ) {
      return { icon: "⚡", text: "Consider short-term trade" };
    }

    if (
      (impactValue === "positive impact" || impactValue === "neutral impact") &&
      riskValue === "low risk"
    ) {
      return { icon: "👀", text: "Hold and observe" };
    }

    return { icon: "⛔", text: "Avoid high volatility entry" };
  };

  const investorSuggestion = result
    ? {
      ...getInvestorSuggestion(result.impact, result.impact_strength, result.risk_level),
      reason: getReasonLine(result.impact_strength, result.risk_level),
      reevaluate: getReevaluationLine(result.impact_strength, result.risk_level)
    }
    : null;
    ...
    <>
        ...
        {result && (
          <>
            ...
            <section className="card suggestion-card">
              <div className="card-glow" />
              <h3>📦 Investor Suggestion</h3>
              <div className="suggestion-body">
                <div className="suggestion-icon">
                  {investorSuggestion?.icon}
                </div>
                <div className="suggestion-content">
                  <p className="suggestion-text">
                    {investorSuggestion?.text}
                  </p>
                  <p className="suggestion-reason">
                    <span>Reason:</span> {investorSuggestion?.reason}
                  </p>
                  <p className="suggestion-reeval">
                    <span>Re-evaluate when:</span> {investorSuggestion?.reevaluate}
                  </p>
                </div>
              </div>
              <p className="suggestion-disclaimer">
                This is a rule-based decision-support layer, not financial advice.
              </p>
            </section>
          </>
        )}
      ...
      <style jsx>{`
        * {
        ...
        .suggestion-card { animation-delay: 0.6s; }
