...
# ---------------------------------------
# 🔥 DASHBOARD EVENT IMPACT ENGINE
# ---------------------------------------

from backend.app.services.news.news_sentiment import analyze_news_sentiment_for_symbol


def detect_event_impacts(symbols=None):
    """
    Detects high-impact market events per stock for dashboard display.
    """
    ...
    for symbol in symbols:
        ...
        if news["market_impact"] in ("High", "Medium"):
            event_text = news["summary"]
            ...
            if result.get("ok"):
                events.append({
                    "symbol": symbol,
                    "event": news["themes"][0] if news["themes"] else "Market News",
                    "impact": news["market_impact"],
                    "confidence": round(result["confidence_percent"] / 100, 2)
                })
