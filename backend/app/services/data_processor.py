        ...
        try:
            ...
            if price and prev_close:
                ...
                return {
                  ...
                  "price": price or 0,
                  "change": change or 0,
                  "changePercent": round(change_percent, 2) if prev_close else 0,
                  "volume": volume or 0
                }
            return None
        except Exception as e:
...
# --------------------------------------
# 🔥 SINGLE SOURCE FOR INDICATORS
# --------------------------------------

def fetch_price_series(symbol: str, days: int = 120) -> pd.Series:
    """
    Returns clean close-price series for indicator calculations.
    Used by RSI, trend, MACD, etc.
    """
    try:
        ...
        df = processor.fetch_stock_data(symbol, start_date, end_date)
        ...
        print(f"[fetch_price_series ERROR] {symbol}: {e}")
