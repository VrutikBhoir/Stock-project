...
# ⚠️ LAZY IMPORT: Heavy statsmodels import deferred to avoid freeze at import time
# These will be loaded on first use by lazy-loading functions

# Global cache for lazy imports
_statsmodels_cache = {}

def _lazy_import_statsmodels():
    """Lazy import all statsmodels dependencies on first use."""
    global _statsmodels_cache
    if not _statsmodels_cache:
        from statsmodels.tsa.arima.model import ARIMA
        from statsmodels.tsa.statespace.sarimax import SARIMAX
        from statsmodels.tsa.stattools import adfuller
        from statsmodels.stats.diagnostic import acorr_ljungbox
        
        _statsmodels_cache['ARIMA'] = ARIMA
        _statsmodels_cache['SARIMAX'] = SARIMAX
        _statsmodels_cache['adfuller'] = adfuller
        _statsmodels_cache['acorr_ljungbox'] = acorr_ljungbox
...
class ModelTrainer:
    ...
    def check_stationarity(self, data: pd.Series) -> bool:
        try:
            statsmodels_mods = _lazy_import_statsmodels()
            adfuller = statsmodels_mods['adfuller']
    ...
    def find_best_sarima_order(self, data: pd.Series, max_p: int = 2, max_d: int = 1, max_q: int = 2, seasonal_periods: int = 12):
        try:
            statsmodels_mods = _lazy_import_statsmodels()
            SARIMAX = statsmodels_mods['SARIMAX']
    ...
    def validate_model(self, data: pd.Series) -> Dict[str, Any]:
        try:
            statsmodels_mods = _lazy_import_statsmodels()
            acorr_ljungbox = statsmodels_mods['acorr_ljungbox']
