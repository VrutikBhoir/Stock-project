    ...
    for test_case in TEST_CASES:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ {symbol}: {sentiment} (conf: {confidence}%) | {conviction} conviction")
                print(f"   Model: {model}")
                
                # Verify no mock values
                if confidence and conviction:
                    print(f"   ✓ ML-driven (not mock)")
                else:
                    print(f"   ❌ Missing ML outputs")
                    
            elif response.status_code in [404, 400]:
                print(f"⚠️  {symbol}: Data not available - {response.json().get('detail')}")
            else:
                print(f"❌ {symbol}: Error {response.status_code}")
                print(f"   {response.text[:200]}")
                
        except Exception as e:
            print(f"❌ {symbol}: Request failed - {str(e)[:100]}")
...
def test_model_loaded():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ Cannot verify - API returned {response.status_code}")
            
    except Exception as e:
        print(f"❌ Cannot verify - {str(e)[:100]}")

if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("🤖 ML-DRIVEN NARRATIVE ENGINE TESTS")
    print("=" * 70)
